...
```{r Average number of steps by interval}
...
The total number of missing data is `r x`
...
```{r Histogram of Total number of steps by day with new values}
...
```{r Average number of steps by interval with new values}
avgStepsByInterval2 <- aggregate(calculatedSteps ~ interval + dayType, imputedData, mean)

library(ggplot2)

ggplot(avgStepsByInterval2, aes(interval,calculatedSteps))  + facet_grid(.~dayType) + labs(x = "Interval") + labs(y = "Average Calculated Steps") + geom_point() +  geom_smooth() 
```
